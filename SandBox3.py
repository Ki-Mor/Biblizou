...
def process_csv(file_path: Path) -> pd.DataFrame:
    ...
    AnneeDerniereObservation = pd.to_datetime(df['Année_DernièreObservation'], errors='coerce').dt.year.tolist()
...
def fuzzy_match_taxa_with_cache(nom_tax: str) -> Tuple[str, int]:
    ...
    for attempt in range(3):
        try:
            response = session.get(url, headers={'accept': "application/hal+json;version=1"}, timeout=25)
...
cache_lock = Lock()  # Verrou pour synchroniser l'accès au cache

def get_taxref_data(CD_Ref, cache={}):
    # Vérifier si les données sont déjà en cache avec verrouillage
    with cache_lock:
        if CD_Ref in cache:
            return cache[CD_Ref]
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if data:
                ...
                # Stocker dans le cache avec verrouillage
                with cache_lock:
                    cache[CD_Ref] = result
                return result
            else:
...
def enrich_global_df():
    global global_df
    logging.info("Enrichissement du DataFrame avec les données de TaxRef :")

    # Liste unique des CD_Ref à interroger
    unique_cd_ref = set(global_df['CD_Ref']) - {0}  # Éviter les 0

    # Appliquer la parallélisation pour enrichir global_df
    with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
        futures = {
            executor.submit(get_taxref_data, cd_ref, cache_taxref_results): cd_ref
            for cd_ref in unique_cd_ref
            if cd_ref not in cache_taxref_results
        }

        for future in concurrent.futures.as_completed(futures):
            cd_ref = futures[future]
            try:
                result = future.result()
                with cache_lock:
                    cache_taxref_results[cd_ref] = result
            except Exception as e:
                logging.error(f"Erreur lors de la récupération des données pour {cd_ref}: {e}")
