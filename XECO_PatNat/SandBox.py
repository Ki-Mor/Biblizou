...
batch_size = 50  # Vous pouvez ajuster cette taille selon la limite autorisée
...
# Créer la table pivotée avec uniquement 'Année_DernièreObservation' et 'Commune'
pivot_df_communes = global_df.pivot_table(
    index=['CD_Ref', 'NomTaxCBNB'],  # Utilisez 'CD_Ref' et 'NomTaxCBNB' comme indices
    columns='Commune',  # Les colonnes seront les différentes communes
    values='Année_DernièreObservation',  # Les valeurs seront 'Année_DernièreObservation'
    aggfunc='max',  # Pour chaque combinaison, on prend le max de l'année
    fill_value=''  # Remplacer les NaN par des valeurs vides
)

# Remise en forme pour obtenir une table lisible
pivot_df_communes = pivot_df_communes.reset_index()

# Fusionner les données de statut avec la table pivotée des communes
df_status_merged = pivot_df_communes.merge(df_status[['id', 'statusTypeName', 'statusCode']],
                                           left_on='CD_Ref', right_on='id', how='left')

# Afficher la table fusionnée avec les informations de statut
