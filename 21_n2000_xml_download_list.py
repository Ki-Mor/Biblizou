...
Name : 21_n2000_xml_download_list
Group : Biblio_PatNat
"""
...
def obtain_folder_path():
    return sys.argv[1] if len(sys.argv) > 1 else input("Entrez le chemin du dossier contenant le fichier input_xml_n2000_download_list.txt : ")
...
def construct_url_and_download(txt_file_path, download_folder):
    ...
    """
    try:
        # Lire et nettoyer le fichier TXT contenant les identifiants N2000
        with open(txt_file_path, 'r', encoding='utf-8') as f:
            n2000_ids = list(filter(None, (id.strip() for id in f.readlines())))
...
def main():
    ...
    # Définir le chemin complet vers le fichier d'entrée (input_xml_n2000_download_list.txt)
    txt_file_path = os.path.join(folder_path, "input_xml_n2000_download_list.txt")

    # Vérifier si le fichier input_xml_n2000_download_list.txt existe dans ce dossier
    if not os.path.isfile(txt_file_path):
        logging.error(f"Le fichier 'input_xml_n2000_download_list.txt' n'a pas été trouvé dans le dossier {folder_path}.")
        return
